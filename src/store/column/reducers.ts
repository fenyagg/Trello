...
import {
  ADD_COLUMN, Card, ColumnActionTypes,
  ColumnState,
...
export default function columns (store: ColumnState[] = columnsData, action: ColumnActionTypes): ColumnState[] {
  ...
  switch (action.type) {
    ...
    case SWAP_COLUMNS:
    {
      ...
      const draggedColumnIndex = immutableStore.findIndex((column: any) => {
        return column.get('id') === draggedColumnId
      });
      const overColumnIndex = immutableStore.findIndex((column: any) => {
    ...
    case SWAP_CARDS:
    {
      ...
      const draggingCardColumnIndex = immutableStore.findIndex((column: any) => {
        const tempCardIndex = column.get('cards').findIndex((card: any) => {
      ...
      const overCardColumnIndex = immutableStore.findIndex((column: any) => {
        const tempCardIndex = column.get('cards').findIndex((card: any) => {
      ...
      const nextStore = immutableStore
        ...
        .updateIn([overCardColumnIndex, 'cards'], (cardsList: any) => {
    ...
    case MOVE_CARD_TO_COLUMN_START:
    {
      ...
      const pushColumnIndex = immutableStore.findIndex((column: any) => column.get('id') === columnId);

      let cardIndex = -1;
      const cardColumnIndex = immutableStore.findIndex((column: any) => {
        const tempCardIndex = column.get('cards').findIndex((card: any) => card.get('id') === cardId);
      ...
      return immutableStore
        ...
        .updateIn([pushColumnIndex, 'cards'], (cardsList: any) => cardsList.unshift(draggingCard))
        .toJS()
    }
    case MOVE_CARD_TO_COLUMN_END:
    {
      ...
      return immutableStore
        .deleteIn([cardColumnIndex, 'cards', cardIndex])
        .updateIn([pushColumnIndex, 'cards'], (cardsList: any) => cardsList.push(draggingCard))
        .toJS()
    }
    case REMOVE_CARD:
    {
    ...
    case REMOVE_COLUMN:
    {
      ...
      const columnIndex = immutableStore.findIndex((column: any) => column.get('id') === columnId);
